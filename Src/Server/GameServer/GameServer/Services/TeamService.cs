...
namespace GameServer.Services
{
    class TeamService : Singleton<TeamService>
    {
        ...
        private void OnTeamInviteRequest(NetConnection<NetSession> sender, TeamInviteRequest request)
        {
            ...
            Log.InfoFormat("OnTeamInviteRequest: :FromId:{0} FromName:{2} ToID:{2} ToName:{3}", request.FromId, request.FromName, request.ToId, request.ToName);
            //TODO:执行一些前置数据校验
            ...
            if (target.Session.Character.Team != null)
            {
                sender.Session.Response.teamInviteRes = new TeamInviteResponse();
                sender.Session.Response.teamInviteRes.Result = Result.Failed;
                sender.Session.Response.teamInviteRes.Errormsg = "该玩家已加入其它队伍";
                sender.SendResponse();
                return;
            }
            //转发请求
        ...
        {
            Character character = sender.Session.Character;
            Log.InfoFormat("OnTeamInviteResponse: :character:{0} Result:{1} FromId:{1} FromName:{2}", character.Id, response.Result, response.Request.FromId, response.Request.FromName);
