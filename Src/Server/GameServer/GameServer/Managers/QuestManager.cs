    ...
    class QuestManager
    {
        ...
        public Result SubmitQuest(NetConnection<NetSession>sender, int questId)
        {
            ...
            if (DataManager.Instance.Quests.TryGetValue(questId, out quest))
            {
                ...
                if (dbquest != null)
                {
                    if (dbquest.Status != (int)QuestStatus.Complated)//任务条件达成，但未提交
                    {
                        //任务是未完成状态
                ...
                {
                    sender.Session.Response.questSubmit.Errormsg = "数据库中该任务不存在[2]";
                    return Result.Failed;
                }
            }
            else
            {
                sender.Session.Response.questSubmit.Errormsg = "任务表中该任务不存在[1]";
                return Result.Failed;
            }
        }
    }
