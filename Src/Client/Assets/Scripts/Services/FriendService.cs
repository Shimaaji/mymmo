...
namespace Services
{
    class FriendService : Singleton<FriendService>, IDisposable
    {
        ...
        public void SendFriendAddResponse(bool accept, FriendAddRequest request)
        {
            ...
            message.Request.friendAddRes.Errormsg = accept ? "对方同意" : "对方拒绝了您的请求";
        ...
        private void OnFriendAddResponse(object sender, FriendAddResponse message)
        {
            if (message.Result == Result.Success)
                MessageBox.Show(message.Request.ToName + "接受了您的好友申请", "添加好友成功");
            else
        ...
        {
            Debug.LogFormat("SendFriendRemoveRequest :myID:{0}, FriendId:{1}",id,friendId);
            NetMessage message = new NetMessage();
            message.Request = new NetMessageRequest();
